...
IDIP_HANDLER_MODIFY_IMPLEMETION(UpdateDragonCoin)
INT32 CMSIdipUpdateDragonCoinHandler::HandlerRequest(const rapidjson::Value& roHeadJson, const rapidjson::Value& roBodyJson, CIdipReply& roReply)
{
...
//修改数据
bool CMSIdipUpdateDragonCoinHandler::ModifyMsData(CUserData& roUserData)
{
	RoleAllInfo* pInfo = (RoleAllInfo*)roUserData.m_pUserPtr;
	CBag data(pInfo);
	data.ReduceVirtualItem(DRAGON_COIN, (m_nNum));
	data.Save();
	return true;
}

void CMSIdipUpdateDragonCoinHandler::IdipLog()
{
	...
	if (GetCmd() == IDIP_DO_GM_UPDATE_DRAGON_GOLD_REQ)
	{
		TIdipDoGmMoneyLog oLog("IdipDoGmUpdateDragonCoinFlow");
		oLog.m_nNum = m_nNum;
		oLog.Do(doc["head"], doc["body"]);
	}
	else
	{
		TIdipMoneyLog oLog("IdipUpdateDragonCoinFlow");
