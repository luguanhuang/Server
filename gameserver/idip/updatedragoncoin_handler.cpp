...
bool CIdipUpdateDragonCoinHandler::Process(const rapidjson::Value& roHeadJson, const rapidjson::Value& roBodyJson, CIdipReply& roReply)
{
	...
	if (nNumber > pBag->CountVirtualItem(DRAGON_COIN))
	{
		nNumber = pBag->CountVirtualItem(DRAGON_COIN);
	}
	BagTakeItemTransition take(pRole);
	take.SetReason(ItemFlow_IDIP, ItemFlow_IDIP_Oper);
	if(!take.TakeItem(DRAGON_COIN, nNumber))
	{
		take.RollBack();
