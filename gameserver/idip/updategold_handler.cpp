...
bool CIdipUpdateGoldHandler::Process(const rapidjson::Value& roHeadJson, const rapidjson::Value& roBodyJson, CIdipReply& roReply)
{
	...
	if (nNumber > pBag->CountVirtualItem(GOLD))
	{
		nNumber = pBag->CountVirtualItem(GOLD);
	}
	BagTakeItemTransition take(pRole);
	take.SetReason(ItemFlow_IDIP, ItemFlow_IDIP_Oper);
	if(!take.TakeItem(GOLD, nNumber))
	{
		take.RollBack();
