#ifndef __PartnerShopTable_H__
#define __PartnerShopTable_H__

#include "pch.h"
#include "cvsreader.h"
#include "config.h"
#include "noncopyable.h"

// generate by ProtoGen at date: 2017/4/14 20:19:25


class PartnerShopTable : public CVSReader, public noncopyable
{
public:

	struct RowData
	{
		uint	ID;
		Sequence<uint, 2>	Slot;
		uint	ItemId;
		uint	ItemNum;
		Sequence<uint, 2>	ConsumeItem;
		Sequence<uint, 2>	LevelShow;
		Sequence<uint, 2>	Benefit;
		uint	LevelCondition;
		int	IsNotBind;

		void CopyFrom(RowData *pRowData)
		{
			this->ID=pRowData->ID;
			this->Slot=pRowData->Slot;
			this->ItemId=pRowData->ItemId;
			this->ItemNum=pRowData->ItemNum;
			this->ConsumeItem=pRowData->ConsumeItem;
			this->LevelShow=pRowData->LevelShow;
			this->Benefit=pRowData->Benefit;
			this->LevelCondition=pRowData->LevelCondition;
			this->IsNotBind=pRowData->IsNotBind;

		}
	};

	std::unordered_map<uint,int> ID_Index;


    RowData *GetByID(const uint &key)
    {
        auto i = ID_Index.find(key);
        return (i == ID_Index.end()) ? NULL : Table[i->second];
    }
    

	virtual bool OnHeaderLine(char **Fields, int FieldCount)
	{
		const char *Headers[] = { "ID", "Slot", "ItemId", "ItemNum", "ConsumeItem", "LevelShow", "Benefit", "LevelCondition", "IsNotBind",  };
		const int HeadersCount = sizeof(Headers)/sizeof(Headers[0]);
		//assert(FieldCount == HeadersCount);
		return MapColHeader(Headers, HeadersCount, Fields, FieldCount);
	}

	virtual bool OnCommentLine(char **Fields, int FieldCount)
	{
		return true;
	}

	void CopyFrom(PartnerShopTable  *pSrcTable)
	{
		for (UINT32 i = 0; i< pSrcTable->Table.size(); i++)
		{
			RowData* pSrcRow = pSrcTable->Table[i];
            RowData *pDstRow = GetByID(pSrcRow->ID);
            if(pDstRow != NULL)
            {
                pDstRow->CopyFrom(pSrcRow);
            }
            else
            {
                RowData* pNewRow = new RowData();
                pNewRow->CopyFrom(pSrcRow);
                Table.push_back(pNewRow);
                ID_Index[pNewRow->ID]=Table.size()-1;
            }

		}
	}

	virtual bool OnLine(char **Fields, int FieldCount)
	{
		RowData *pRow = new RowData();
		RowData &row = *pRow;
		m_ColNum = 0;

		if (!Parse(Fields[m_ColMap[0]], row.ID)) return false; ++m_ColNum;
		if (!Parse(Fields[m_ColMap[1]], row.Slot)) return false; ++m_ColNum;
		if (!Parse(Fields[m_ColMap[2]], row.ItemId)) return false; ++m_ColNum;
		if (!Parse(Fields[m_ColMap[3]], row.ItemNum)) return false; ++m_ColNum;
		if (!Parse(Fields[m_ColMap[4]], row.ConsumeItem)) return false; ++m_ColNum;
		if (!Parse(Fields[m_ColMap[5]], row.LevelShow)) return false; ++m_ColNum;
		if (!Parse(Fields[m_ColMap[6]], row.Benefit)) return false; ++m_ColNum;
		if (!Parse(Fields[m_ColMap[7]], row.LevelCondition)) return false; ++m_ColNum;
		if (!Parse(Fields[m_ColMap[8]], row.IsNotBind)) return false; ++m_ColNum;

		
		Table.push_back(pRow);

		ID_Index[row.ID] = Table.size()-1;


		return true;
	}

	~PartnerShopTable ()
	{
		Clear();
	}

	void Clear()
	{
		for (UINT32 i = 0; i < Table.size(); ++i)
		{
			delete Table[i];
		}

		Table.clear();

		ID_Index.clear();

	}

	std::vector<RowData*> Table;
};

#endif