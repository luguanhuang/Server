#ifndef __QALevelRewardTable_H__
#define __QALevelRewardTable_H__
...
class QALevelRewardTable : public CVSReader, public noncopyable
{
public:
	...
	{
		const char *Headers[] = { "ID", "QAType", "MinLevel", "MaxLevel", "Reward", "ExtraReward",  };
	...
	void CopyFrom(QALevelRewardTable  *pSrcTable)
	{
	...
	virtual bool OnLine(char **Fields, int FieldCount)
	{
		...
		if (!Parse(Fields[m_ColMap[3]], row.MaxLevel)) return false; ++m_ColNum;
		if (!Parse(Fields[m_ColMap[4]], row.Reward)) return false; ++m_ColNum;
		if (!Parse(Fields[m_ColMap[5]], row.ExtraReward)) return false; ++m_ColNum;
