#ifndef __RechargeTable_H__
#define __RechargeTable_H__
...
class RechargeTable : public CVSReader, public noncopyable
{
public:
	...
	struct RowData
	{
		...
		int	Price;
		int	Diamond;
		vector<Sequence<int, 2>>	RoleLevels;
		vector<Sequence<int, 2>>	LoginDays;
		int	SystemID;
		string	Name;
		string	ServiceCode;
		...
		void CopyFrom(RowData *pRowData)
		{
			...
			this->RoleLevels=pRowData->RoleLevels;
			this->LoginDays=pRowData->LoginDays;
	...
	{
		const char *Headers[] = { "ParamID", "Price", "Diamond", "RoleLevels", "LoginDays", "SystemID", "Name", "ServiceCode",  };
	...
	void CopyFrom(RechargeTable  *pSrcTable)
	{
	...
	virtual bool OnLine(char **Fields, int FieldCount)
	{
		...
		if (!Parse(Fields[m_ColMap[3]], row.RoleLevels)) return false; ++m_ColNum;
		if (!Parse(Fields[m_ColMap[4]], row.LoginDays)) return false; ++m_ColNum;
		if (!Parse(Fields[m_ColMap[5]], row.SystemID)) return false; ++m_ColNum;
		if (!Parse(Fields[m_ColMap[6]], row.Name)) return false; ++m_ColNum;
		if (!Parse(Fields[m_ColMap[7]], row.ServiceCode)) return false; ++m_ColNum;
