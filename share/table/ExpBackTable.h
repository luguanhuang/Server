#ifndef __ExpBackTable_H__
#define __ExpBackTable_H__
...
class ExpBackTable : public CVSReader, public noncopyable
{
public:
	...
	struct RowData
	{
		...
		void CopyFrom(RowData *pRowData)
		{
			...
			this->freeExpParam=pRowData->freeExpParam;
			this->moneyCostParam=pRowData->moneyCostParam;
	...
	{
		const char *Headers[] = { "type", "count", "exp", "freeExpParam", "moneyCostParam",  };
	...
	void CopyFrom(ExpBackTable  *pSrcTable)
	{
	...
	virtual bool OnLine(char **Fields, int FieldCount)
	{
		...
		if (!Parse(Fields[m_ColMap[3]], row.freeExpParam)) return false; ++m_ColNum;
		if (!Parse(Fields[m_ColMap[4]], row.moneyCostParam)) return false; ++m_ColNum;
