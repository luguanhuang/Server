#ifndef __TaJieHelpTab_H__
#define __TaJieHelpTab_H__

#include "pch.h"
#include "cvsreader.h"
#include "config.h"
#include "noncopyable.h"

// generate by ProtoGen at date: 2017/7/24 15:56:23


class TaJieHelpTab : public CVSReader, public noncopyable
{
public:

	struct RowData
	{
		uint	Index;
		vector<uint>	Type;
		uint	SysID;
		string	IconName;
		string	Des;
		string	Name;

		void CopyFrom(RowData *pRowData)
		{
			this->Index=pRowData->Index;
			this->Type=pRowData->Type;
			this->SysID=pRowData->SysID;
			this->IconName=pRowData->IconName;
			this->Des=pRowData->Des;
			this->Name=pRowData->Name;

		}
	};

	std::unordered_map<uint,int> Index_Index;


    RowData *GetByIndex(const uint &key)
    {
        auto i = Index_Index.find(key);
        return (i == Index_Index.end()) ? NULL : Table[i->second];
    }
    

	virtual bool OnHeaderLine(char **Fields, int FieldCount)
	{
		const char *Headers[] = { "Index", "Type", "SysID", "IconName", "Des", "Name",  };
		const int HeadersCount = sizeof(Headers)/sizeof(Headers[0]);
		//assert(FieldCount == HeadersCount);
		return MapColHeader(Headers, HeadersCount, Fields, FieldCount);
	}

	virtual bool OnCommentLine(char **Fields, int FieldCount)
	{
		return true;
	}

	void CopyFrom(TaJieHelpTab  *pSrcTable)
	{
		for (UINT32 i = 0; i< pSrcTable->Table.size(); i++)
		{
			RowData* pSrcRow = pSrcTable->Table[i];
            RowData *pDstRow = GetByIndex(pSrcRow->Index);
            if(pDstRow != NULL)
            {
                pDstRow->CopyFrom(pSrcRow);
            }
            else
            {
                RowData* pNewRow = new RowData();
                pNewRow->CopyFrom(pSrcRow);
                Table.push_back(pNewRow);
                Index_Index[pNewRow->Index]=Table.size()-1;
            }

		}
	}

	virtual bool OnLine(char **Fields, int FieldCount)
	{
		RowData *pRow = new RowData();
		RowData &row = *pRow;
		m_ColNum = 0;

		if (!Parse(Fields[m_ColMap[0]], row.Index)) return false; ++m_ColNum;
		if (!Parse(Fields[m_ColMap[1]], row.Type)) return false; ++m_ColNum;
		if (!Parse(Fields[m_ColMap[2]], row.SysID)) return false; ++m_ColNum;
		if (!Parse(Fields[m_ColMap[3]], row.IconName)) return false; ++m_ColNum;
		if (!Parse(Fields[m_ColMap[4]], row.Des)) return false; ++m_ColNum;
		if (!Parse(Fields[m_ColMap[5]], row.Name)) return false; ++m_ColNum;

		
		Table.push_back(pRow);

		Index_Index[row.Index] = Table.size()-1;


		return true;
	}

	~TaJieHelpTab ()
	{
		Clear();
	}

	void Clear()
	{
		for (UINT32 i = 0; i < Table.size(); ++i)
		{
			delete Table[i];
		}

		Table.clear();

		Index_Index.clear();

	}

	std::vector<RowData*> Table;
};

#endif